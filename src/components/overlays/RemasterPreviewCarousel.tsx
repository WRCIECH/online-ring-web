...
import { describeRemasterStates, DRAW_LABEL_KEY, VALUE_BUCKET, type RemasterSlotView } from '../../data/weaponStructure'
...
function renderSlot(slot: RemasterSlotView, key: string, t: TranslationBundle) {
  ...
  return (
    ...
    >
      {t.ui[DRAW_LABEL_KEY[slot.kind]] ?? slot.kind}
      {': '}
...
export default function RemasterPreviewCarousel({ weapon }: Props) {
  ...
  const pageLabel = page === 0 ? t.ui.remaster_preview_current : `${t.ui.remaster_preview_step} ${page}`
