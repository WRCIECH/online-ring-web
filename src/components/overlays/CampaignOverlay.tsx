...
export default function CampaignOverlay({ onClose }: Props) {
  ...
  function renderEdgeLabel(label: CampaignEdge['label']) {
  ...
  function renderTree(
    ...
    return nodeIds.map(nodeId => {
      ...
      const canEdit = !node.completed   // all non-completed nodes are renameable
      ...
      return (
        <div key={node.id} className={s.treeItem}>
          {/* Edge label sits above the node row for non-root nodes */}
          {incomingEdge !== undefined && (
            <div className={s.edgeRow}>{renderEdgeLabel(incomingEdge.label)}</div>
          )}

          {/* Horizontal connector line + node row */}
          {depth > 0 ? (
            <div className={s.nodeWithConnector}>
              <div className={s.hLine} aria-hidden="true" />
              {nodeRowEl}
            </div>
          ) : nodeRowEl}
          ...
          {children.length > 0 && (
            <div className={s.childrenGroup}>
              {renderTree(children, nodesMap, childrenMap, edges, allNodes, weaponId, depth + 1)}
            </div>
          )}
        </div>
      )
